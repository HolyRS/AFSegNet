...
class _GN(nn.GroupNorm):
    def __init__(self, c):  # 适配任意通道
        g = 8 if c % 8 == 0 else (4 if c % 4 == 0 else 1)
        super().__init__(g, c)

class DirectionHead(nn.Module):
    """小头：预测二维方向场 u=(ux,uy)，小 batch 用 GN 更稳"""
    def __init__(self, in_ch, mid=32):
        super().__init__()
        hid = max(8, mid)
        self.net = nn.Sequential(
            nn.Conv2d(in_ch, mid, 3, padding=1), _GN(mid), nn.SiLU(),
            nn.Conv2d(mid, hid, 3, padding=1),   _GN(hid), nn.SiLU(),
            nn.Conv2d(hid, 2, 3, padding=1)
        )
    def forward(self, x):  # x:(B,D,H,W)
        return self.net(x) # (B,2,H,W)

class OFGOptimal(nn.Module):
    """
    稳定版方向门控：
      - 线性升温：kappa: k0 → k1（warmup_epochs）
      - 残差强度：lambda: 0 → lam_max（warmup_epochs）
      - 置信度回退：m=tanh(alpha*||u||)
      - top-2 稀疏：从 start_topk_epoch 起
      - 早期断梯度：epoch < detach_until_epoch 时对 feat_2d.detach()
      - 能量守恒：重标 g，使 sum_k g = K（mean=1）
    """
    def __init__(self,
                 in_ch,
                 kappa_init=1.0, kappa_target=2.5, warmup_epochs=10,
                 lam_max=0.5, start_topk_epoch=20, topk=2,
                 gmin=0.05, conf_alpha=0.75, detach_until_epoch=15):
        super().__init__()
        self.dir_head = DirectionHead(in_ch)
        self.kappa_init = float(kappa_init)
        self.kappa_target = float(kappa_target)
        self.warmup_epochs = int(warmup_epochs)
        self.lam_max = float(lam_max)
        self.start_topk_epoch = int(start_topk_epoch)
        self.topk = int(topk)
        self.gmin = float(gmin)
        self.conf_alpha = float(conf_alpha)
        self.detach_until_epoch = int(detach_until_epoch)

        # 4 个“无向”原型方向：水平、垂直、主对角、反对角（与你的 CrossScan 顺序一致）
        phi = torch.tensor([0., math.pi/2, math.pi/4, 3.*math.pi/4])
        basis = torch.stack([torch.cos(phi), torch.sin(phi)], dim=0)  # (2,4)
        self.register_buffer("basis", basis.view(1,2,4,1,1))          # (1,2,4,1,1)
        self.register_buffer("epoch", torch.tensor(0, dtype=torch.long))
        self.register_buffer("lam", torch.tensor(0., dtype=torch.float))

    def set_epoch(self, e:int):
        self.epoch.fill_(int(e))
        t = min(max(e, 0), self.warmup_epochs) / max(1, self.warmup_epochs)
        self.lam.fill_(self.lam_max * t)

    @property
    def kappa(self):
        t = min(self.epoch.item(), self.warmup_epochs) / max(1, self.warmup_epochs)
        return self.kappa_init + t * (self.kappa_target - self.kappa_init)

    def _map_g4_to_K(self, g4, K, H, W, device, dtype):
        if K == 4:
            return g4
        elif K == 8:
            gK = torch.zeros(g4.size(0), 8, H, W, device=device, dtype=dtype)
            # 0/2: 水平 正/反；1/3: 垂直；4/6: 主对角；5/7: 反对角（与你的 CrossScan 对齐）
            gK[:,0] = g4[:,0]; gK[:,2] = g4[:,0]
            gK[:,1] = g4[:,1]; gK[:,3] = g4[:,1]
            gK[:,4] = g4[:,2]; gK[:,6] = g4[:,2]
            gK[:,5] = g4[:,3]; gK[:,7] = g4[:,3]
            ...
            raise ValueError(f"OFG: unexpected K={K}")

    def forward(self, feat_2d, ys_kchw):
        """
        feat_2d: (B,D,H,W) —— 用于预测方向
        ys_kchw: (B,K,C,H,W) —— K 向 SSM 输出（K=4 或 8）
        return:  同形状，已门控
        """
        assert ys_kchw.dim()==5, f"OFG expects (B,K,C,H,W), got {ys_kchw.shape}"
        B,K,C,H,W = ys_kchw.shape
        assert feat_2d.shape[-2:]==(H,W), "OFG: feat_2d HW must match ys"

        # 早期断梯度更稳
        feat_in = feat_2d.detach() if (self.training and self.epoch.item()<self.detach_until_epoch) else feat_2d

        # 方向场 → 单位向量
        uv  = self.dir_head(feat_in)                     # (B,2,H,W)
        mag = uv.norm(dim=1, keepdim=True).clamp_min(1e-6)
        v   = uv / mag

        # 余弦打分 + softmax
        scores = self.kappa * (v.unsqueeze(2) * self.basis).sum(dim=1)  # (B,4,H,W)
        g4 = F.softmax(scores, dim=1)

        # 置信度回退到均匀（低‖u‖ 更均匀）
        m = torch.tanh(self.conf_alpha * mag)            # (B,1,H,W)
        g4 = m * g4 + (1 - m) * 0.25

        # top-k 稀疏（延后启用）
        if self.training and self.epoch.item() >= self.start_topk_epoch and self.topk < 4:
            topv, topi = torch.topk(g4, k=self.topk, dim=1)
            mask = torch.zeros_like(g4).scatter_(1, topi, 1.0)
            g4 = (g4 * mask) / (g4 * mask).sum(1, keepdim=True).clamp_min(1e-6)

        # 4 → K（或 K=4 直接用），并做能量守恒与下限
        gK = self._map_g4_to_K(g4, K, H, W, ys_kchw.device, ys_kchw.dtype)
        gK = self.gmin + (1.0 - self.gmin) * gK          # 下限，防压没
        scale = K / gK.sum(1, keepdim=True).clamp_min(1e-6)  # sum→K (mean=1)
        g_mean1 = (gK * scale).to(dtype=ys_kchw.dtype)   # 与 ys 同 dtype（FP16 友好）

        # 残差门控（恒等起步，warmup 后逐步生效）
        lam = float(self.lam.item())
        ys_kchw.mul_(1.0 + lam * (g_mean1.unsqueeze(2) - 1.0))  # 就地乘法，省显存/更快
        with torch.no_grad():
            self.last_gate_sum = float(g_mean1.sum(1).mean().item())
            probs = (gK / gK.sum(1, keepdim=True)).clamp_min(1e-6)
            self.last_gate_entropy = (-(probs * probs.log()).sum(1) / math.log(probs.shape[1])).mean().item()
            self.last_mag = float(mag.mean().item())
        return ys_kchw
...
def cross_selective_scan(
    ...
    # ---------- 新增：在 CrossMerge 之前做门控 ----------
    if ofg is not None:
        # x: (B,D,H,W) 就是用来预测方向的 2D 特征；ys: 8 向 SSM 输出
        ys = ofg(feat_2d=x, ys_kchw=ys)
...
class OSSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        del self.x_proj
        # ===== 在 OSSM.__init__ 末尾附近加入 =====
        # 门控模块（通道用 d_inner）
        self.ofg = OFGOptimal(
            in_ch=int(ssm_ratio * d_model),
            kappa_init=1.0, kappa_target=2.8 , warmup_epochs=20,
            lam_max=0.4, start_topk_epoch=40, topk=2,
            gmin=0.08, conf_alpha=0.6, detach_until_epoch=20
        )
        # out proj =======================================
...
class OSSBlock(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        ...
        if self.use_checkpoint:
            return checkpoint.checkpoint(self._forward, input).permute(0, 3, 1, 2)
        else:
            return self._forward(input).permute(0, 3, 1, 2)
