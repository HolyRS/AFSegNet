...
class GSS2D(nn.Module):
    def __init__(self, in_channels):
        ...
        # 方向单位向量基（固定，不训练）
        phi = torch.tensor([0., 3*math.pi/2, math.pi, math.pi/2])
        basis = torch.stack([torch.cos(phi), torch.sin(phi)], dim=0)  # (2,4)
        self.register_buffer("basis", basis.view(1, 2, 4, 1, 1))

        self.kappa = 2.0
        self.conf_alpha = 0.75
        self.use_confidence = True

    def forward(self, feat_2d, ys_4chw):
        # feat_2d: (B,D,H,W)
        # ys_4chw: (B,4,C,H,W)

        uv = self.dir_head(feat_2d)    # (B,2,H,W)
        mag = uv.norm(dim=1, keepdim=True)  # (B,1,H,W)
        v = uv / (mag + 1e-6)

        # 余弦相似度得分 (B,4,H,W)
        scores = self.kappa * (v.unsqueeze(2) * self.basis).sum(dim=1)
        g4 = F.softmax(scores, dim=1)

        if self.use_confidence:
            m = torch.tanh(self.conf_alpha * mag)
            g4 = m * g4 + (1 - m) * 0.25

        gated = ys_4chw * g4.unsqueeze(2)
        print(g4)
        return gated
"""""
class GSS2D(nn.Module):
    ...
    输出：
        gated: (B, 4, C, H, W)
    """
    def __init__(self, in_channels, kappa=2.0, conf_alpha=0.75, use_confidence=True):
        super().__init__()
        self.kappa = kappa
        ...
        # 输出 2*C 通道，表示每个 channel 的方向向量 (ux, uy)
        self.dir_head = nn.Sequential(
            ...
            nn.BatchNorm2d(32),
            nn.SiLU(),
            nn.Conv2d(32, 2 * in_channels, 3, padding=1),  # 每个 C 通道都输出 ux, uy
        )
        ...
        # 注册为 buffer
        self.register_buffer("basis", basis.view(1, 1, 2, 4, 1, 1))  # (1,1,2,4,1,1)

    def forward(self, feat_2d, ys_4chw):
        B, C, H, W = feat_2d.shape

        # --- 预测每个 channel 的二维方向向量 (B, 2*C, H, W) ---
        uv = self.dir_head(feat_2d)                        # (B, 2*C, H, W)
        uv = uv.view(B, C, 2, H, W)                         # (B, C, 2, H, W)

        # --- 方向归一化 ---
        mag = uv.norm(dim=2, keepdim=True)                  # (B, C, 1, H, W)
        v = uv / (mag + 1e-6)                               # (B, C, 2, H, W)

        # --- 与四个方向基对齐 ---
        # v:      (B, C, 2, H, W)
        # basis:  (1, 1, 2, 4, 1, 1)
        # → 输出: (B, C, 4, H, W)
        scores = (v.unsqueeze(3) * self.basis).sum(dim=2)   # (B, C, 4, H, W)
        scores = self.kappa * scores

        # --- Softmax over 4 directions ---
        g4 = F.softmax(scores, dim=2)                       # (B, C, 4, H, W)

        # --- 置信度控制（每个通道独立） ---
        if self.use_confidence:
            m = torch.tanh(self.conf_alpha * mag)           # (B, C, 1, H, W)
            g4 = m * g4 + (1 - m) * 0.25

        # --- 广播成 (B,4,C,H,W) ---
        g4 = g4.permute(0, 2, 1, 3, 4)                      # (B,4,C,H,W)
        # --- 对 ys_4chw 进行方向加权 ---
        gated = ys_4chw * g4                                # (B,4,C,H,W)

        return gated

"""""
def gss2d(feat_2d, ys_4chw,
          kappa=2.0,
          use_confidence=True,
          conf_alpha=0.75):
    
    GSS2D: Gated Selective Scan 2D (函数版)
    --------------------------------------
    feat_2d : (B, D, H, W)
        输入特征，用于预测方向场
    ys_4chw : (B, 4, C, H, W)
        4 向扫描输出特征 (来自 selective_scan)
    kappa : float
        控制 softmax 的尖锐程度（越大越“硬”）
    use_confidence : bool
        是否基于置信度进行门控平滑
    conf_alpha : float
        控制 tanh(α * ||u||) 的幅度
    ...
    dir_head = nn.Sequential(
        nn.Conv2d(D, 32, 3, padding=1),
        nn.BatchNorm2d(32),
        nn.SiLU(),
        nn.Conv2d(32, 16, 3, padding=1),
        nn.BatchNorm2d(16),
        nn.SiLU(),
        nn.Conv2d(16, 2, 3, padding=1)   # 输出 u_x, u_y
    ).to(feat_2d.device)

    uv = dir_head(feat_2d)     # (B,2,H,W)
    mag = uv.norm(dim=1, keepdim=True)  # (B,1,H,W)
    v = uv / (mag + 1e-6)

    # ---------- 4个方向的单位向量基 ----------
    phi = torch.tensor([0., 3*math.pi/2, math.pi, math.pi/2],
                       device=feat_2d.device, dtype=feat_2d.dtype)
    basis = torch.stack([torch.cos(phi), torch.sin(phi)], dim=0).view(1, 2, 4, 1, 1)  # (1,2,4,1,1)

    # ---------- 计算余弦相似度得分 ----------
    scores = kappa * (v.unsqueeze(2) * basis).sum(dim=1)  # (B,4,H,W)
    g4 = F.softmax(scores, dim=1)  # 每像素4个方向概率

    # ---------- 根据置信度平滑 ----------
    if use_confidence:
        m = torch.tanh(conf_alpha * mag)
        g4 = m * g4 + (1 - m) * 0.25  # 退回均匀分布

    # ---------- 应用门控 ----------
    gated = ys_4chw * g4.unsqueeze(2)  # 广播乘
    return gated
"""""
# pytorch cross scan =============
...
def cross_selective_scan(
    ...
    #print(y.shape)
    #print(y.shape)
    # 将四个方向的结果 reshape 回 (B,4,C,H,W)
    #ys_4chw = y.view(B, 4, D, H, W)
    #gated = gss2d(x, ys_4chw)
    #y = gated.sum(1)
    if channel_first:
        y = y.view(B, -1, H, W)
...
class SS2D(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        #self.gss2d = GSS2D(in_channels=d_model)
    ...
    def __initv0__(
        ...
        **kwargs,
        ...
        #self.gss2d = GSS2DVector(in_ch=d_model)
        ...
        self.gss2d = GSS2D(in_channels=d_inner)
